...
public class Gato {
    ...
    LinkedList<Gato> generaSucesores() {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                if (tablero[i][j] == 0) { // No se ha tirado en esa posición.
                    // Se construye un nuevo gato que tenga las mismas tiradas que en el que estamos trabajando.
                    Gato aux = new Gato(this); 
                    aux.jugador1 = !jugador1;
                    aux.tiradas = tiradas + 1;

                    aux.tiraEn(i, j); // Se tira en esa posición para generar un sucesor.

                    if (!yaEsta(sucesores, aux)) // Se ve que no esté en la lista de sucesores contemplando simetrías.
                        sucesores.add(aux);
                }
    ...
    /**
     * Revisa si dado una lista de Gatos y uno en específico, este ya se 
     * encuentra o no en la lista contemplando simetrías.
     */
    boolean yaEsta(LinkedList<Gato> sucesores, Gato gato) {
    ...
    boolean esSimetrico90(Gato otro) {
        // Composición entre las simetrías diagonal \ y horizontal:
        for (int i = 0; i < 3; i++)
           for (int j = 0; j < 3; j++)
    ...
    boolean esSimetrico180(Gato otro) {
        // Composición entre las simetrías horizontal y vertical:
        for (int i = 0; i < 3; i++)
           for (int j = 0; j < 3; j++)
    ...
    boolean esSimetrico270(Gato otro) {
        // Composición entre las simetrías diagonal \ y vertical:
        for (int i = 0; i < 3; i++)
           for (int j = 0; j < 3; j++)
